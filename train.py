...
def main(opt):
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        train(epoch, G, D, target_net, criterionGAN, clf_criterion, metric_criterion, optimizer_G, optimizer_D,
              trainloader, use_gpu)
        ...
        if epoch % args.eval_freq == 0:
            ...
            if args.ak_type < 0:
                cmc, mAP = test(G, D, test_target_net, dataset, queryloader, galleryloader, epoch, use_gpu,
                                is_test=False)
                is_worst = cmc[0] <= worst_rank1 and cmc[1] <= worst_rank5 and cmc[
                    2] <= worst_rank10 and mAP <= worst_mAP
                if is_worst:
                    worst_mAP, worst_rank1, worst_epoch = mAP, cmc[0], epoch
                print("==> Worst_epoch is {}, Worst mAP {:.1%}, Worst rank-1 {:.1%}".format(worst_epoch, worst_mAP,
                                                                                            worst_rank1))
...
def train(epoch, G, D, target_net, criterionGAN, clf_criterion, metric_criterion, optimizer_G, optimizer_D, trainloader,
          use_gpu):
    G.train()
    D.train()
    global is_training
    ...
    for batch_idx, (imgs, pids, _, pids_raw) in enumerate(trainloader):
        ...
        if args.loss in ['cent', 'xent', 'xent_htri']:
            if args.ak_type < 0:
                xent_loss = DeepSupervision(clf_criterion, new_outputs, pids) if isinstance(new_features, (
                tuple, list)) else clf_criterion(new_outputs, pids)
        ...
        if args.loss in ['htri', 'xent_htri']:
            assert len(ls) >= 2
            global_loss = DeepSupervision(metric_criterion, new_features, pids, targets) if isinstance(new_features, (
            tuple, list)) else metric_criterion(new_features, pids, targets)
        ...
        if (batch_idx + 1) % args.print_freq == 0:
            print(
                "===> Epoch[{}]({}/{}) loss_D: {:.4f} loss_G_GAN: {:.4f} loss_G_ReID: {:.4f} loss_G_SSIM: {:.4f}".format(
                    epoch, batch_idx, len(trainloader), loss_D.item(), loss_G_GAN.item(), loss_G_ReID.item(),
                    loss_G_ssim))
    ...
    with torch.no_grad():
        ...
        if args.ak_type > 0:
            ...
            if is_test:
                ...
                if args.usevis:
                    visualize_ranked_results(new_distmat, dataset, save_dir=osp.join(vis_dir, 'polluted_results'),
                                             topk=20)
            else:
                _, new_cmc, new_mAP = make_results(new_qf, gf, new_lqf, lgf, q_pids, g_pids, q_camids, g_camids,
                                                   args.targetmodel, args.ak_type)
                print("mAP: {:.1%}, Rank-{}: {:.1%}, Rank-{}: {:.1%}, Rank-{}: {:.1%}, Rank-{}: {:.1%}".format(new_mAP,
                                                                                                               ranks[0],
                                                                                                               new_cmc[
                                                                                                                   ranks[
                                                                                                                       0] - 1],
